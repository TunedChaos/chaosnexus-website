...
All plugins start with **no** capabilities. Host config `[plugins.<name>].granted_capabilities` must authorize the plugin; effective caps are the intersection of that host grant and `plugin.toml` `[capabilities]`. Forge approval writes the host grant.
...
- Module imports rooted at `plugins/` and `lib/` (no bare CWD/`FileModuleResolver::new()`)
...
|------|------------------|--------|
| `run_command` | argv `run_command(exec, args)` by default; legacy shell `-c` gated by `CHAOSNEXUS_ANVIL_ALLOW_SHELL_C` | Keep argv-only in plugins; avoid granting interpreter bins with `-c` |
| `http_get` / `http_post` | Host allowlist + no redirects; private/loopback blocked unless net-star opt-in | Pin resolved IPs / harden DNS-rebinding further |
