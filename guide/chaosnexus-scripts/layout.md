...
Logical **scripts root** (workspace / polyrepo checkout):
...
`scripts_dir` must be this root, not `plugins/` alone (unless your tooling treats `plugins` as a child of the configured root). workspace default from `chaosnexus-anvil/`:
...
- workspace `tools/launch/` helpers
