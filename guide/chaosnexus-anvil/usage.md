...
By default, ChaosNexus Anvil expects your Rhai scripts under a scripts root (`chaosnexus-scripts/` in the workspace, or any path set via `scripts_dir` / `--scripts-dir`). Ensure plugins live under that root's `plugins/` directory before execution.
